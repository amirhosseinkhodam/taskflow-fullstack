...
- **Update `backend-explained.md` after backend edits**: Whenever any file under `backend/` is created, edited, or deleted, update `doc/backend-explained.md` to reflect the changes. Add documentation for new modules/endpoints, update descriptions for modified ones, and remove entries for deleted ones. This keeps the backend documentation in sync with the actual code.

## Documentation structure

- `doc/` — TaskFlow-only documentation (plans, security, design system, testing plan, READMEs).
- `doc/backend-explained.md` — Backend explained for frontend developers.
